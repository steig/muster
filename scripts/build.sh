...
if [ -z "$expected" ]; then
	echo "herdr-wt: no checksum published for $asset; refusing to install it" >&2
...
if command -v sha256sum >/dev/null 2>&1; then
	...
	echo "herdr-wt: no sha256 tool to verify the download; install Go and retry" >&2
	rm -f "$OUT" bin/checksums.txt
	exit 1
fi

if [ "$expected" != "$actual" ]; then
	echo "herdr-wt: checksum mismatch for $asset" >&2
